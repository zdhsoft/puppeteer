 ...
 * well as all that are defined on {@link PuppeteerNode}. @public
 */
export class Puppeteer {
  /**
   * Registers a {@link CustomQueryHandler | custom query handler}.
   *
   * @remarks After registration, the handler can be used everywhere where a
   * selector is expected by prepending the selection string with `<name>/`. The
   * name is only allowed to consist of lower- and upper case latin letters.
   *
   * @example
   *
   * ```
   ...
   * under. @param queryHandler - The {@link CustomQueryHandler | custom query
   * handler} to register.
   *
   * @public
   */
