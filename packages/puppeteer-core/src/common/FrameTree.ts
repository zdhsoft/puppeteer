...
/**
 * Keeps track of the page frame tree and it's is managed by {@link
 * FrameManager}. FrameTree uses frame IDs to reference frame and it means that
 * referenced frames might not be in the tree anymore. Thus, the tree structure
 * is eventually consistent. @internal
 */
export class FrameTree {
  ...
  /**
   * Returns a promise that is resolved once the frame with the given ID is
   * added to the tree.
   */
