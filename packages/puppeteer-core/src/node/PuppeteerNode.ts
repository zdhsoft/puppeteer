 ...
 *
 * @remarks The most common method to use is {@link PuppeteerNode.launch |
 * launch}, which is used to launch and connect to a new browser instance.
 *
 * See {@link Puppeteer | the main Puppeteer class} for methods common to all
 * environments, such as {@link Puppeteer.connect}.
 *
 * @example The following is a typical example of using Puppeteer to drive
 * automation:
 *
 * ```ts
 ...
 * with the page, navigate, or find certain elements in that page. The {@link
 * Page | `page` documentation} lists all the available methods.
 *
 * @public
 */
export class PuppeteerNode extends Puppeteer {
   ...
   *
   * @example You can use `ignoreDefaultArgs` to filter out `--mute-audio` from
   * default arguments:
   *
   * ```ts
   * const browser = await puppeteer.launch({
   ...
   * ```
   *
   * @remarks **NOTE** Puppeteer can also be used to control the Chrome browser,
   * but it works best with the version of Chromium it is bundled with. There is
   * no guarantee it will work with any other version. Use `executablePath`
   * option with extreme caution. If Google Chrome (rather than Chromium) is
   * preferred, a {@link https://www.google.com/chrome/browser/canary.html |
   * Chrome Canary} or {@link
   * https://www.chromium.org/getting-involved/dev-channel | Dev Channel} build
   * is suggested. In `puppeteer.launch([options])`, any mention of Chromium
   * also applies to Chrome. See {@link
   * https://www.howtogeek.com/202825/what%E2%80%99s-the-difference-between-chromium-and-chrome/
   * | this article} for a description of the differences between Chromium and
   * Chrome. {@link
   * https://chromium.googlesource.com/chromium/src/+/lkgr/docs/chromium_browser_vs_google_chrome.md
  ...
  /**
   * @remarks **NOTE** `puppeteer.executablePath()` is affected by the
   ...
   * `"firefox"`)
   *
   * @remarks The product is set by the `PUPPETEER_PRODUCT` environment variable
   * or the `product` option in `puppeteer.launch([options])` and defaults to
   * `chrome`. Firefox support is experimental.
   */
  get product(): string {
   ...
   * BrowserFetcher. @returns A new BrowserFetcher instance.
   */
